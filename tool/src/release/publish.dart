...
void stageReleaseAssets({required String distDir, required String outputDir}) {
  ...
  for (final entity in dist.listSync(recursive: true)) {
    if (entity is! File) continue;
    if (!entity.path.endsWith('.glucoseplugin')) continue;
  ...
  if (copied.isEmpty) {
    stderr.writeln('No .glucoseplugin files found in $distDir');
    for (final entity in dist.listSync(recursive: true)) {
      stderr.writeln(entity.path);
    }
    throw StateError('No .glucoseplugin files found in downloaded artifacts');
  }

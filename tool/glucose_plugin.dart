...
import 'package:glucosebar_plugins/plugin_manifest.dart';
import 'package:glucosebar_plugins/platform_id.dart';
...
/// GlucoseBar plugin authoring CLI.
///
/// ```bash
...
Future<void> _cmdCreate(List<String> args) async {
  ...
  final options = parseScaffoldOptions(
    identifier: id,
    displayName: name,
    author: parsed.flags['author'] ?? 'GlucoseBar',
...
Commands:
  ...
  build     Compile, sign (macOS), write dist/<id>.glucoseplugin (--no-package to skip)
  package   Build dist/<id>.glucoseplugin (after compile)

Install .glucoseplugin files via the glucosebar app repo:
  dart run tool/install_plugin.dart path/to/plugin.glucoseplugin

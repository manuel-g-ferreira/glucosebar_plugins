...
- `userId` in GlucoseBar session = **follower account id**, not patient id
...
Map to GlucoseBar plugin trend strings per [PLUGIN-PROTOCOL-V1.md](PLUGIN-PROTOCOL-V1.md).
...
Default poll interval 5 min (GlucoseBar) aligns with DiaKEM stream default 90s — avoid sub-minute polling.
...
**Recommendation for GlucoseBar LibreLink plugin:** Follow **DiaKEM `readRaw` + `read` split** internally; one graph HTTP call; expose standard five host commands.

# Writing a GlucoseBar CGM plugin

Anyone can ship a plugin if it respects the **connection points** — no approval from GlucoseBar required.
...
GlucoseBar does **not** run Python inside the app. Your plugin is still a **separate program** that reads **one JSON line from stdin** and writes **one JSON line to stdout**. That is a few lines in Python.
...
GlucoseBar runs the binary — it does not know it was Python. Alternatives: `cx_Freeze`, `py2app`, **Nuitka**.
...
|----|-------------------|
| macOS | `~/Library/Application Support/GlucoseBar/Plugins/` |
| Windows | `%AppData%\GlucoseBar\Plugins\` |
| Linux | `~/.local/share/GlucoseBar/plugins/` |

Ship a **`.glucoseplugin`** package (directory bundle, not zip). Build with:

```bash
dart run tool/glucose_plugin.dart build plugins/YourPlugin
```

End users drag the package into **Settings → Plugins** or use **Choose .glucoseplugin…**.

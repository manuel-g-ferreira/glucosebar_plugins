...
Use **`dart run tool/glucose_plugin.dart`** from the repo root. It scaffolds new plugins, compiles binaries, and writes **`dist/<id>.glucoseplugin`** — one executable file per platform (not a folder or zip).
...
`plugins/MyCgm/dist/mycgm.glucoseplugin`
...
## Install in GlucoseBar

End users: drag **`dist/<id>.glucoseplugin`** into **Settings → Plugins** on any OS. No terminal required.

Developers: `dart run tool/glucose_plugin.dart build plugins/<Name>` produces `dist/<id>.glucoseplugin`.

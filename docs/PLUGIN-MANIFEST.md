...
|-------|------|-------------|
| `identifier` | string | Unique slug, e.g. `librelink` (package folder: `librelink.glucoseplugin`) |
| `displayName` | string | Shown in Settings → Plugins |
| `version` | string | Semver, e.g. `1.0.0` |
| `apiVersion` | string | Must be `"1"` for current GlucoseBar |
...
| `minHostVersion` | string | GlucoseBar semver if needed later |
...
On macOS the same folder may be named `Example CGM.glucoseplugin` (directory bundle).

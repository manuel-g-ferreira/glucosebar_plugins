...
GlucoseBar CGM plugin for **LibreLink Up** (caregiver / follower accounts).
...
Output: `plugins/LibreLink/dist/librelink.glucoseplugin` (one executable file, like `LibreLink` in GlucoseBar_mac)

## Install into GlucoseBar

Drag **`dist/librelink.glucoseplugin`** into **Settings → Plugins**, or use **Choose plugin file…**.

Restart GlucoseBar → **Reload** → **Data Source** to sign in.
